...
class MainWindow(Ui_MainWindow):
    def __init__(self):
        ...
        self.inn_company_line.setValidator(QIntValidator())
        self.inn_company_line_change.setValidator(QIntValidator())
        self.inn_line.setValidator(QIntValidator())
        self.inn_line_change.setValidator(QIntValidator())
    ...
    def on_selection_changed_3(self):
        ...
        if selected_indexes:
            ...
            for column in range(model.columnCount()):
                index = model.index(row, column)
                data.append(model.data(index))
            self.year_quta_change.setText(str(data[3]))
            self.year_tax_change.setText(str(data[4]))
